...
@register_model
def downsample_more(num_classes: int, rgb_input: bool = False) -> nn.Module:
    input_channels = 3 if rgb_input else 1
    conv_block_1 = nn.Sequential(
        nn.Conv2d(input_channels, 16, 5, stride=2, padding=1, bias=False),
        nn.BatchNorm2d(16),
        nn.LeakyReLU(),
    )
    conv_block_2 = nn.Sequential(
    ...
    conv_block_7 = nn.Sequential(
        nn.Conv2d(128, 128, 5, padding=1, stride=2),
        nn.LeakyReLU(),
    )
    conv_block_8 = nn.Conv2d(128, 5 + num_classes, 1)
    return nn.Sequential(
...
@register_model
def downsample_longer_more(num_classes: int, rgb_input: bool = False) -> nn.Module:
    input_channels = 3 if rgb_input else 1
    conv_block_1 = nn.Sequential(
        nn.Conv2d(input_channels, 16, 5, stride=2, padding=1, bias=False),
        nn.BatchNorm2d(16),
    ...
    conv_block_7 = nn.Sequential(
        nn.Conv2d(128, 128, 5, padding=1, stride=2, bias=True),
        nn.BatchNorm2d(128),
        nn.LeakyReLU(),
    )
...
@register_model
def downsample_more_bigger_kernels(num_classes: int, rgb_input: bool = False) -> nn.Module:
    input_channels = 3 if rgb_input else 1
    conv_block_1 = nn.Sequential(
