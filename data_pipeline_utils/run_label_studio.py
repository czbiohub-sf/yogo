...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "path to run set folder (`<some path>/scope-parasite-data/run-sets` on flexo), defaults to running on OnDemand if no argument is provided. used for serving images only."
        ),
